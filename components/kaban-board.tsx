...
function DroppableColumn({
  ...
  return (
    <Card className="min-w-[285px] max-w-[285px] flex-shrink-0 shadow-md p-0 mx-auto" >
      ...
      >
        <SortableContext
          items={sortedJobApp.map((job) => job._id)}
...
export default function KabanBoard({ board, userId }: KanbanBoardProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    let sourceColumnId: Column | null = null;
    let sourceIndex = -1;
    ...
    //check if over is column or job
    ...
    if (targetColumn) {
      ...
      const targetColumnObj = sortedColumns.find(
        (cols) => cols._id === targetColumnId,
      );
      if (!targetColumnObj) return;

      const allJobsInTargetOrignal =
        targetColumnObj.jobApplications.sort((a, b) => a.order - b.order) || [];

      const allJobsInTargetFiltered =
        allJobsInTargetOrignal.filter((job) => job._id !== activeId) || [];

      const targetIndexInOrginal = allJobsInTargetOrignal.findIndex(
        (job) => job._id === overId,
      );
      ...
      if (targetIndexInFiltered !== -1) {
        if (sourceColumnId._id === targetColumnId) {
          if (sourceIndex < targetIndexInOrginal) {
    ...
    await moveJob(draggedJob._id, targetColumnId, newOrder);
  }
